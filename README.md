...
#### Параметры ФМР (упрощенная настройка)
```python
GYROMAGNETIC_RATIO = 2.8e-3  # Гиромагнитное отношение (ГГц/Э)

# Калибровочная точка (удобнее чем поле анизотропии)
FMR_CALIBRATION_FIELD = 3000      # Поле, при котором известна частота (Э)
FMR_CALIBRATION_FREQUENCY = 8.4   # Резонансная частота при этом поле (ГГц)
```

#### Параметры потерь (упрощенная модель)
```python
CAVITY_LINEWIDTH = 1.0    # Полная ширина линии резонатора (ГГц)
MAGNON_LINEWIDTH = 0.14   # Полная ширина линии магнонной моды (ГГц)
```

> **Преимущества**: Вместо настройки отдельно внутренних и внешних потерь, теперь достаточно указать наблюдаемую ширину линии каждой моды.
